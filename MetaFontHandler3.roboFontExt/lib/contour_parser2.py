...
def contour_parser2(wd):
	...
	for glif in glif_list:
		if glif.endswith('.plist'):
			...
			c = open(cho+joong+jong + '_' + joong + '.glif', 'w')
			while True:
				line = f.readline()
				if '<outline>' in line:
					c.write(amolang_start.replace("putnamehere", cho+joong+jong + '_' + joong))
					c.write(line)
			...
			tree = parse(cho+joong+jong + '_' + joong + '.glif')
	...
	#							else:
				...
				ElementTree(glif_root).write(cho+joong+jong + '_' + joong + '.glif')
